...
const deleteTask = async (req, res) => {
    ...
    try {
        // Check if the user has permission to delete this task (e.g., project owner or task creator)
        const permissionCheck = await db.query(
            `SELECT t.id FROM tasks t JOIN projects p ON t.project_id = p.id
             WHERE t.id = $1 AND p.created_by = $2`, // Only project creator can delete tasks
            [id, user_id]
        );
...
const addTagToTask = async (req, res) => {
    ...
    try {
        // Ensure the task exists and the user has permission to modify it
        ...
        // Add the tag to the task_tags junction table
